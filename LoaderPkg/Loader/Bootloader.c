...
InitGraphics(
    ...
    //   DEBUG ((DEBUG_INFO, "Res: %u; Idx: %u\n", GraphicsOutput->Mode->Info->HorizontalResolution, i));
    // }

    GraphicsOutput->SetMode(GraphicsOutput, 18);
...
GetKernelFile(
    ...
    // LAB 1: Your code here
    KernelFile = NULL;
    Status = CurrentDriveRoot->Open(CurrentDriveRoot, &KernelFile, KERNEL_PATH, EFI_FILE_MODE_READ, EFI_FILE_VALID_ATTR);
    if (EFI_ERROR(Status)) {
        DEBUG((DEBUG_ERROR, "JOS: Cannot access own file system - %r\n", Status));
        return Status;
    }
    ...
    if (!EFI_ERROR(Status)) {
        ...
        for (Index = 0; Index < ElfHeader.e_phnum; ++Index) {
            if (ProgramHeaders[Index].p_type == PT_LOAD) {
                ...
                //
                MinAddress = MinAddress < ProgramHeaders[Index].p_pa ? MinAddress : ProgramHeaders[Index].p_pa;
                MaxAddress = (MaxAddress > ProgramHeaders[Index].p_pa + ProgramHeaders[Index].p_memsz) ? MaxAddress : ProgramHeaders[Index].p_pa + ProgramHeaders[Index].p_memsz;
            }
        }
    ...
    if (!EFI_ERROR(Status)) {
        ...
        *EntryPoint = (UINTN)ElfHeader.e_entry;
        ...
        for (Index = 0; Index < ARRAY_SIZE(mDebugMapping); ++Index) {
            SectionData = (VOID *)(UINTN) * (EFI_PHYSICAL_ADDRESS *)((UINT8 *)LoaderParams + mDebugMapping[Index2].StartOffset);

            if (SectionData != NULL) {
                gBS->FreePages(
                        (UINTN)SectionData,
                        EFI_SIZE_TO_PAGES(
                                *(EFI_PHYSICAL_ADDRESS *)((UINT8 *)LoaderParams + mDebugMapping[Index2].EndOffset) - *(EFI_PHYSICAL_ADDRESS *)((UINT8 *)LoaderParams + mDebugMapping[Index2].StartOffset)));
            }
        }
    }
